    ...
    checkinToHtml = function(val) {
        info = <<
          <div style="margin: 20px">
            ...
            <ul>
              <li><span>Venue: #{val.pick("$.venue")}</span></li>
              <li><span>City: #{val.pick("$.city")}</span></li>
              <li><span>Shout: #{val.pick("$.shout")}</span></li>
              <li><span>Created At: #{val.pick("$.createdAt")}</span></li>
            </ul>
          </div>
        >>;
        info;
